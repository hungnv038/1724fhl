<div class="list-group">
    ...
    @foreach($packages as $package)
        ...
        ?>
        <a href="#" class="list-group-item" id="package_id_{{$package->id}}" data-pid="{{$package->id}}" onclick="getPackageItems(this,{{$package->id}},'page'); return false;"><span class="badge">{{$package->counter}}</span>{{$package->package_name}}<span class="badge badge-info">{{$package->language}}</span>{{$strNew}}</a>
    @endforeach
</div>