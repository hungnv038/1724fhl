...
class Movie extends DBAccess {
    ...
    public function getByChanelId($chanel_id,$since,$limit) {
        if($chanel_id==17) {
            $last_chanel=" and to_days(now())- to_days(chanel_movie.created_at)<=2";
        } else {
            $last_chanel="";
        }

        $sql="select movie.id,chanel_movie.chanel_id,title,url,movie.match_url,thumb,
              unix_timestamp(chanel_movie.created_at) as created_at,
              unix_timestamp(chanel_movie.updated_at) as updated_at from chanel_movie
              inner join movie on movie.match_url=chanel_movie.match_url
              where chanel_movie.chanel_id=? and unix_timestamp(chanel_movie.created_at) < ? {$last_chanel}
              order by chanel_movie.created_at DESC
              limit 0, ?";
